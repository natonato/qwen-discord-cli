...
program
  .name('ai-discord')
  .description('Bridge AI CLI (Qwen/Gemini) with Discord channels')
  .version('1.0.0');

program
  .command('start')
  .description('Start the AI-Discord bridge bot')
  .action(async () => {
    try {
      console.log('🚀 Starting AI-Discord Bridge...');
...
program
  ...
  .action(() => {
    try {
      ...
      console.log(`  - Approval Mode: ${config.approvalMode}`);
      console.log(`  - Working Directory: ${config.workingDir}`);
      console.log(`  - AI Timeout: ${config.aiTimeoutMs / 1000}s`);
